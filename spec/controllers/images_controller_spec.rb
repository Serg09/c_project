...
RSpec.describe ImagesController, type: :controller do
  let (:unapproved_image) { FactoryBot.create(:image) }
  let (:approved_bio_image) { FactoryBot.create(:image) }
  let!(:bio) { FactoryBot.create(:approved_bio, photo_id: approved_bio_image.id) }
  let (:approved_book_image) { FactoryBot.create(:image) }
  let!(:book_version) { FactoryBot.create(:approved_book_version, cover_image_id: approved_book_image.id) }
